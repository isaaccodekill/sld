...
export default function ReportPage() {
  ...
  return (
    <div className="space-y-5">
      <nav className="flex flex-wrap items-center justify-between gap-3 print:hidden"><Link href="/admin/reports" className="text-sm text-ink-3 hover:text-ink">← Reports</Link><div className="flex items-center gap-2"><Tag tone={report.markedSharedAt ? "good" : "warn"}>{report.markedSharedAt ? "Shared" : "Draft"}</Tag><Button variant="outline" onClick={() => window.print()}>Print / Save PDF</Button></div></nav>
