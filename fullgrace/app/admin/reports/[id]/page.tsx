...
export default function ReportPage() {
  ...
  return (
    <div className="space-y-5">
      <nav className="flex flex-wrap items-center justify-between gap-3 print:hidden"><Link href="/admin/reports" className="text-sm text-ink-3 hover:text-ink">← Reports</Link><div className="flex flex-wrap items-center gap-2"><LinkButton href={`/admin/reports/${report.id}/edit`} variant="outline" size="sm" className="min-w-[118px]">Edit report</LinkButton><Button variant="outline" size="sm" onClick={() => window.print()}>Print / Save PDF</Button></div></nav>
      <article className="mx-auto min-h-[850px] max-w-[820px] rounded-2xl border border-line bg-white p-6 shadow-[0_18px_60px_rgba(23,35,45,.06)] sm:p-10 md:p-14 print:border-0 print:p-0 print:shadow-none">
        <header className="border-b border-line pb-7"><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-green">Fullgrace Children Therapy Centre</p><div className="mt-3 flex flex-wrap items-center gap-3"><h1 className="font-display text-3xl sm:text-4xl">{report.title || report.occasionNote || "Progress report"}</h1><Tag className="print:hidden" tone={report.markedSharedAt ? "good" : "warn"}>{report.markedSharedAt ? "Shared" : "Draft"}</Tag></div><div className="mt-4 flex flex-wrap gap-x-6 gap-y-1 text-sm text-ink-2"><span><strong>Child:</strong> {client.firstName}</span><span><strong>Period:</strong> {formatDate(report.windowStart)} – {formatDate(report.windowEnd)}</span></div></header>
